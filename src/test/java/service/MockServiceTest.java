...
@RunWith(MockitoJUnitRunner.class)//Mockito framework을 초기화하기 위해 사용
public class MockServiceTest {
    ...
    @Test
    public void 챔피언_이름을_가져오면_무조건_카이사를_리턴한다() {
        Champion champion = mock(Champion.class); //mock객체 생성
    ...
    public void shouldReturnVayneWhenAnyChampionName(){
        ...
        System.out.println("Champion : " + champion.getName()); //null값이나온다.
    ...
    public void shouldExceptionWhenSetNameYasuo(){
        Champion champion = mock(Champion.class);
        doThrow(new IllegalArgumentException()).when(champion).setName("아스오");
        champion.setName("아스오");
    }


    // 3. verify 를 사용하여 '미드' 포지션을 저장하는 프로세스가 진행되었는지 테스트 하세요.
    @Test
    public void shouldSetMidPositionUserVerify(){
        Champion champion = mock(Champion.class);
        champion.setName("직스");
    ...
    public void confirmArraySizeCheck(){
        ...
        System.out.println("Size :: " + mockChampions.size());//mock객체이기 떄문에 실제 값은 없으므로 0으로 나온다.
    ...
    public void shouldTwoTimesInvocationForGetChampionName(){
        ...
        verify(champion, atLeast(2)).getName();//최소 2번 이상
    }

    // 4-2. champion 객체에서 이름을 가져오는 로직이 최대 3번까지 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldAtMostTreeTimesWhenChampionGetName(){
        Champion champion = mock(Champion.class);
        ...
        System.out.println("Name :: " + champion.getName());
        //System.out.println("Name :: " + champion.getName());
        //System.out.println("Name :: " + champion.getName()); //최대 3번까지므로 4번실행시 error
    ...
    @Test
    public void shouldAtLeastOnetimeInvocationIn200msForChampionGetName(){
        Champion champion = mock(Champion.class);
        champion.setName("트페");
        champion.setPosition("미드");
    ...
    @Test
    public void shouldPositionISTopOfForFindBynameLissandra(){
